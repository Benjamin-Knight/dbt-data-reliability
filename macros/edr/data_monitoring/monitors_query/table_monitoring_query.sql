...
{% macro get_no_timestamp_table_query(monitored_table, metric_properties, table_metrics, full_table_name_str) %}
  {% do return(adapter.dispatch("get_no_timestamp_table_query", "elementary")(monitored_table, metric_properties, table_metrics, full_table_name_str)) %}
{% endmacro %}

{% macro default__get_no_timestamp_table_query(monitored_table, metric_properties, table_metrics, full_table_name_str) %}
...
{% macro get_timestamp_table_query(monitored_table, metric_properties, timestamp_column, table_metrics, min_bucket_start, max_bucket_end, full_table_name_str) %}
  {% do return(adapter.dispatch("get_timestamp_table_query", "elementary")(monitored_table, metric_properties, timestamp_column, table_metrics, min_bucket_start, max_bucket_end, full_table_name_str)) %}
{% endmacro %}

{% macro default__get_timestamp_table_query(monitored_table, metric_properties, timestamp_column, table_metrics, min_bucket_start, max_bucket_end, full_table_name_str) %}
...
{% macro get_unified_metrics_query(table_metrics, metric_properties) %}
  {% do return(adapter.dispatch("get_unified_metrics_query", "elementary")(table_metrics, metric_properties)) %}
{% endmacro %}
...
{% macro row_count_metric_query(metric, metric_properties) %}
  {% do return(adapter.dispatch("row_count_metric_query", "elementary")(metric, metric_properties)) %}
{% endmacro %}
...
{% macro freshness_metric_query(metric, metric_properties) %}
  {% do return(adapter.dispatch("freshness_metric_query", "elementary")(metric, metric_properties)) %}
{% endmacro %}
...
{% macro sqlserver__get_timestamp_table_query(monitored_table, metric_properties, timestamp_column, table_metrics, min_bucket_start, max_bucket_end, full_table_name_str) %}
    {# We do not support nested CTEs, we need to output complete chains of named CTEs #}
    with partially_time_filtered_monitored_table as (
        select
            {{ elementary.edr_cast_as_timestamp(timestamp_column) }} as monitored_table_timestamp_column
            {%- if metric_properties.timestamp_column and metric_properties.event_timestamp_column %}
...
{% macro sqlserver__freshness_metric_query(metric, metric_properties) %}
    ...
    from 
        ...
        (
            ...
            select
                *,
                row_number () over (partition by edr_bucket_end order by case when freshness is null then 1 else 0 end, freshness desc) as row_number
            from 
            (
                -- divide the freshness metrics above to buckets
                select * from 
                (
                    ...
                    where 
                        update_timestamp >= edr_bucket_start AND update_timestamp < edr_bucket_end
                ) AS c
                union all
                select * from 
                (
